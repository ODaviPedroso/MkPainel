...
import { api } from './api';
import { GoogleAdsMark, MetaAdsMark, Icon } from './components';

export function OnboardingScreen({ email, onDone }) {
  const [googleStatus, setGoogleStatus] = useState("idle"); // idle | connecting | connected
  ...
  const connectGoogle = async () => {
    ...
    try {
      await api.connectPlatform('google', 'Google Ads MCC');
      setGoogleStatus("connected");
    } catch (err) {
      alert(err.message || 'Erro ao conectar Google Ads');
  ...
  const connectMeta = async () => {
    ...
    try {
      await api.connectPlatform('meta', 'Meta Ads Manager');
      setMetaStatus("connected");
    } catch (err) {
      alert(err.message || 'Erro ao conectar Meta Ads');
      setMetaStatus("idle");
    }
  };
          ...
          <div className={`connect-row ${googleStatus === "connected" ? "connected" : ""}`}>
            ...
            <div className="connect-info">
              ...
              <div className="connect-meta">
                ...
                {googleStatus === "connected" && <span style={{ color: "var(--green-700)" }}>✓ Conectado · Contas importadas com sucesso</span>}
              </div>
            </div>
