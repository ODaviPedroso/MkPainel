...
public class GoogleAdsService : IGoogleAdsService
{
    ...
    public GoogleAdsService(IApplicationDbContext context)
    {
        _context = context;
    }
    ...
    public async Task SyncCampaignsAsync(Guid connectedAccountId, string accessToken)
    {
        ...
        foreach (var campaign in account.Campaigns)
        {
            ...
            {
                decimal addedSpent = (decimal)(random.NextDouble() * 50 + 10);
                int addedClicks = random.Next(10, 100);
                int addedConversions = random.Next(0, 5);
                ...
                decimal newCtr = newClicks > 0 ? (decimal)newClicks / (newClicks * 20) : campaign.Ctr;
                decimal newRoas = newSpent > 0 ? (decimal)(newConversions * 150) / newSpent : campaign.Roas;
