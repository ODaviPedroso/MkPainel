// Shared components for MKPainel

const { useState, useEffect, useRef } = React;

// ─── Logo mark (Option A — painel com "MK" em traço de gráfico) ──────────
...
// ─── Platform marks ──────────────────────────────────────────
function GoogleAdsMark({ size = 22 }) {
  // Original geometric mark — three colored bars representing a chart
  return (
...
function MetaAdsMark({ size = 22 }) {
  // Original mark — overlapping squares for Meta's two platforms
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
...
// ─── Animated number ──────────────────────────────────────────
function AnimNumber({ value, format = "currency", className = "" }) {
...
// ─── Sparkline ──────────────────────────────────────────
function Sparkline({ trend = "up", color, width = 100, height = 32 }) {
  const path = useRef(window.genSparkline(14, trend)).current;
...
function Sidebar({ active, onNav, alertCount }) {
  ...
  // Close after nav on mobile
  const handleNav = (id) => { setOpen(false); onNav?.(id); };

  const items = [
    { id: "dashboard", label: "Visão Geral", icon: Icon.Dashboard },
    { id: "accounts", label: "Contas", icon: Icon.Accounts, count: 20 },
    { id: "compare", label: "Google vs Meta", icon: Icon.Versus, badge: "novo" },
    { id: "campaigns", label: "Campanhas", icon: Icon.Campaigns, count: 142 },
    { id: "funnel", label: "Funil & Retenção", icon: Icon.Funnel },
    { id: "reports", label: "Relatórios", icon: Icon.Report },
  ];
  const items2 = [
      ...
      <aside className={`sidebar ${open ? "open" : ""}`}>
        ...
        <div className="sidebar-foot">
          ...
          <div className="sidebar-foot-info">
            <div className="sidebar-foot-name">Davi Silva</div>
            <div className="sidebar-foot-mail">admin@agenciaorbita.com.br</div>
          </div>
        </div>
      </aside>
    </>
...
Object.assign(window, {
  Icon, GoogleAdsMark, MetaAdsMark, AnimNumber, Sparkline, Sidebar,
});
